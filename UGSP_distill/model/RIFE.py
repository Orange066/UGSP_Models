...
class Model:
    def __init__(self, local_rank=-1, arbitrary=False, epoch = None, count_time = False):
        ...
        self.device()
        self.optimG = AdamW(self.flownet.parameters(), lr=1e-6, weight_decay=0) # use large weight decay may avoid NaN loss
        self.epe = EPE()
        self.lap = LapLoss()
        self.sobel = SOBEL()
    ...
    def inference(self, img0, img1, scale_list=[4, 2, 1], TTA=False, timestep=0.5, count_time = False, use_sparsity=False, use_up_sample =False):
        if count_time == False:
            imgs = torch.cat((img0, img1, img1), 1)
            flow_l, mask, merged, loss_sparsity, check_mask, flow_c_l, imgt_pred_c, feat_l, feat_c_l, gc_loss_l = self.flownet(imgs, timestep=timestep, tau=0.01, use_up_sample = use_up_sample)
            if TTA == False:
    ...
    def update(self, imgs, gt, flow, space_mask_gpu, var_gpu, learning_rate=0, mul=1, training=True, flow_gt=None, epoch = None, tau = None):
        ...
        flow_l, mask, merged, loss_sparsity, check_mask, flow_c_l, merged_c, feat_l, feat_c_l, gc_loss_l = self.flownet(torch.cat((imgs, gt), 1), tau=tau, epoch = epoch)
        ...
        feat_t_0, feat_t_1, feat_t_2, imgt_context_l = gc_loss_l
        loss_geo = 0.01 * (self.gc_loss(feat_t_0, imgt_context_l[1]) + self.gc_loss(feat_t_1,
                                                                                    imgt_context_l[2]) + self.gc_loss(
            feat_t_2, imgt_context_l[3]))
        if flow is not None:
            robust_weight0 = self.get_robust_weight(flow_l[3][:, 0:2], flow[:, 0:2], beta=0.3)
            robust_weight1 = self.get_robust_weight(flow_l[3][:, 2:4], flow[:, 2:4], beta=0.3)
            loss_dis = 0.01 * (self.rb_loss(2.0 * resize(flow_l[2][:, 0:2], 2.0) - flow[:, 0:2],
                                            weight=robust_weight0) + self.rb_loss(
                2.0 * resize(flow_l[2][:, 2:4], 2.0) - flow[:, 2:4], weight=robust_weight1))
            loss_dis += 0.01 * (self.rb_loss(4.0 * resize(flow_l[1][:, 0:2], 4.0) - flow[:, 0:2],
                                             weight=robust_weight0) + self.rb_loss(
                4.0 * resize(flow_l[1][:, 2:4], 4.0) - flow[:, 2:4], weight=robust_weight1))
            loss_dis += 0.01 * (self.rb_loss(8.0 * resize(flow_l[0][:, 0:2], 8.0) - flow[:, 0:2],
                                             weight=robust_weight0) + self.rb_loss(
                8.0 * resize(flow_l[0][:, 2:4], 8.0) - flow[:, 2:4], weight=robust_weight1))
        else:
        ...
        if training:
            ...
            loss_G = loss_l1 * 1. + loss_l1_c * 0.5 + 0.001 * loss_contrast_feat  \
                     + (torch.abs(loss_sparsity - 0.365) * .1 + loss_local_mask_label * w_step ) * lambda_sparsity + loss_geo + loss_dis
