...
class VimeoDataset(Dataset):
    ...
    def crop(self, img0, gt, img1, flow, space_mask,  var, h, w):
        ih, iw, _ = img0.shape
        ...
        return img0, gt, img1, flow, space_mask, var
    ...
    def getimg(self, index):
        ...
        imgpaths = [imgpath + '/im1.png', imgpath + '/im2.png', imgpath + '/im3.png',
                    # imgpath + '/32_guide_mask_0.png', imgpath + '/32_guide_mask_1.png', imgpath + '/32_guide_mask_2.png',
        ...
        flowpaths = [flowpath + '/flow_t0.flo', flowpath + '/flow_t1.flo']
        ...
        timestep = 0.5
        flow_t0 = self.readFlow(flowpaths[0])
        flow_t1 = self.readFlow(flowpaths[1])
        flow = np.concatenate((flow_t0, flow_t1), 2).astype(np.float64)

        var = np.load(imgpaths[6]).astype(np.float64)

        return img0, gt, img1, timestep, flow, space_mask, var


    def __getitem__(self, index):
        img0, gt, img1, timestep, flow, space_mask, var = self.getimg(index)
        # print('img0_0', img0.shape)
        # print('space_0', space_mask.shape)
        # print('var_0', var.shape)
        if self.dataset_name == 'train':
            img0, gt, img1, flow, space_mask, var = self.crop(img0, gt, img1, flow, space_mask, var, 224, 224)
        ...
        return torch.cat((img0, img1, gt), 0), timestep, flow, space_mask, var
