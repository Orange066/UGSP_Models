...
class GLBlock(nn.Module):
    ...
    def forward(self, inputs, tau):
        ...
        # print('feat_0_warp', feat_0_warp.shape)
        # print('feat_1_warp', feat_1_warp.shape)
        # print('feat_dense', feat_dense.shape)
        ...
        # print('x', x.shape)
        # print('self.conv_head', self.conv_head)
...
class IFNet(nn.Module):
    ...
    def forward(self, x, tau=0.01, epoch = None, timestep = None):
        ...
        flow_c_l = [flow_0.clone(), flow_1_c.clone(), flow_2_c.clone(), flow_3_c.clone()]
        feat_l = [feat_t_1.clone(), feat_t_2.clone(), feat_t_3.clone()]
        feat_c_l = [feat_t_1_c.clone(), feat_t_2_c.clone(), feat_t_3_c.clone()]

        # print('loss_global_mask', loss_channel_mask.shape)
        # print('loss_local_mask', loss_local_mask.shape)
        ...
        # loss_sparsity_encoder = torch.ones_like(img0_context_l[0]).view(bs, -1)
        # loss_sparsity_encoder = torch.cat([loss_sparsity_encoder, torch.ones_like(img0_context_l[0]).view(bs, -1)], dim=1)
        # loss_sparsity_encoder = torch.cat([loss_sparsity_encoder, torch.ones_like(img0_context_l[1]).view(bs, -1)], dim=1)
        # loss_sparsity_encoder = torch.cat([loss_sparsity_encoder, torch.ones_like(img0_context_l[1]).view(bs, -1)], dim=1)
        # loss_sparsity_encoder = torch.cat([loss_sparsity_encoder, torch.ones_like(img0_context_l[2]).view(bs, -1)], dim=1)
        # loss_sparsity_encoder = torch.cat([loss_sparsity_encoder, torch.ones_like(img0_context_l[2]).view(bs, -1)], dim=1)
        # loss_sparsity_encoder = torch.cat([loss_sparsity_encoder, torch.ones_like(img0_context_l[3]).view(bs, -1)], dim=1)
        # loss_sparsity_encoder = torch.cat([loss_sparsity_encoder, torch.ones_like(img0_context_l[3]).view(bs, -1)], dim=1)

        # loss_sparsity = torch.cat([loss_sparsity, torch.ones_like(img0_context_l[0]).view(bs, -1)], dim=1)
        # loss_sparsity = torch.cat([loss_sparsity, torch.ones_like(img0_context_l[0]).view(bs, -1)], dim=1)
        # loss_sparsity = torch.cat([loss_sparsity, torch.ones_like(img0_context_l[1]).view(bs, -1)], dim=1)
        # loss_sparsity = torch.cat([loss_sparsity, torch.ones_like(img0_context_l[1]).view(bs, -1)], dim=1)
        # loss_sparsity = torch.cat([loss_sparsity, torch.ones_like(img0_context_l[2]).view(bs, -1)], dim=1)
        # loss_sparsity = torch.cat([loss_sparsity, torch.ones_like(img0_context_l[2]).view(bs, -1)], dim=1)
        # loss_sparsity = torch.cat([loss_sparsity, torch.ones_like(img0_context_l[3]).view(bs, -1)], dim=1)
        # loss_sparsity = torch.cat([loss_sparsity, torch.ones_like(img0_context_l[3]).view(bs, -1)], dim=1)


        # print('sparsity_0', sparsity_0.shape)
        # print('sparsity_1', sparsity_1.shape)
        # print('sparsity_2', sparsity_2.shape)
        # # print('sparsity_3', sparsity_3.shape)
        # print('loss_sparsity', loss_sparsity.shape)
        # # print('sparsity_encoder', loss_sparsity_encoder.shape)
        # #
        # print('0', sparsity_0.shape[1] / loss_sparsity.shape[1])
        # print('1', sparsity_1.shape[1] / loss_sparsity.shape[1])
        # print('2', sparsity_2.shape[1] / loss_sparsity.shape[1])
        # # print('3', sparsity_3.shape[1] / loss_sparsity.shape[1])
        # print('all', (sparsity_0.shape[1] + sparsity_1.shape[1] + sparsity_2.shape[1] ) / loss_sparsity.shape[1])
        # # print('encoder', loss_sparsity_encoder.shape[1] / loss_sparsity.shape[1])
        # #
        # exit((0))
        loss_sparsity = loss_sparsity.mean()
        # print('loss_sparsity',loss_sparsity)

        return flow_l, mask, imgt_pred, loss_sparsity, check_mask, flow_c_l, imgt_pred_c, feat_l, feat_c_l
