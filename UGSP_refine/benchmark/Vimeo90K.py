...
for i_count, i in enumerate(f):
    ...
    if count_time == False:
        ...
        mid = mid[0]
        mid_save = mid.clone()
        mid_save = mid_save.permute(1, 2, 0)
        mid_save = np.round((mid_save * 255).detach().cpu().numpy()).astype('uint8')
        if os.path.exists('./vimeo_compare/') == False:
            os.mkdir('./vimeo_compare/')
        cv2.imwrite('./vimeo_compare/' + name.replace('/', '_') + '.png', mid_save)
        mid_c = mid_c[0]
        ...
        flops = flops_0 + flops_1 + flops_2 + 0.743981056 * 2 + 0.934277568 + 0.15074304 * 2 + 3.781894144
        print('flops', flops)
    else:
