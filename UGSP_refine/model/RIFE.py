...
class Model:
    ...
    def inference(self, img0, img1, scale_list=[4, 2, 1], TTA=False, timestep=0.5, count_time = False, use_sparsity=False):
        if count_time == False:
            ...
            flow_l, mask, merged, loss_sparsity, check_mask, flow_c_l, imgt_pred_c, feat_l, feat_c_l, loss_distill, flow_3_tea, imgt_pred_tea = self.flownet(imgs, timestep=timestep, tau=0.01)
            if TTA == False:
    ...
    def update(self, imgs, gt, space_mask_gpu, var_gpu, learning_rate=0, mul=1, training=True, flow_gt=None, epoch = None, tau = None):
        ...
        flow_l, mask, merged, loss_sparsity, check_mask, flow_c_l, merged_c, feat_l, feat_c_l, loss_distill, flow_gt, merged_t = self.flownet(torch.cat((imgs, gt), 1), tau=tau, epoch = epoch)

        # uncertainty loss
        loss_l1 = (self.lap(merged, gt)).mean()
        loss_l1_c = (self.lap(merged_c, gt)).mean()
        loss_tea = (self.lap(merged_t, gt)).mean()
        ...
        if training:
            ...
            loss_G = loss_l1 * 1. + loss_l1_c * 0.5 + 0.001 * loss_contrast_feat  \
                     + (torch.abs(loss_sparsity - 0.35) * .15 + loss_local_mask_label * w_step) * lambda_sparsity + loss_distill * 0.02 + loss_tea
